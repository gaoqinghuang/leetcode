...
func maxCompatibilitySum(students [][]int, mentors [][]int) int {
	if len(students) !=len(mentors) || len(students)==0 {
		return 0
	}
	maxScore := 0
	allM := getAllSort(mentors)
	score := 0
	for _,m := range allM {
		score = 0
		for i, s := range students {
			score += getScore(m[i],s)
		}
