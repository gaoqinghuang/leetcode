...
//n^2，从大到小排序，时间复杂度是nlogn
func carFleet(target int, position []int, speed []int) int {
	//排序

	//从小到达排序
	pss := make([]*ps,0,len(position))
	...
	sort.Slice(pss, func(i, j int) bool {
		return pss[i].Position<pss[j].Position
	})
	result := len(position)
	//比较
	for i:=0;i<len(pss)-1;i++ {
		for j:=i+1;j<len(pss);j++ {
			if chase(target,pss[i],pss[j]){
				result--
				break
			}
		}
...
//能不能追上
func chase(target int,x,y *ps)  bool{
	return float64((target-x.Position))/float64(x.Speed) <= float64((target-y.Position))/float64(y.Speed)
}