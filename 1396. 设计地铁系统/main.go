...
func main() {
	c:=Constructor()
	c.CheckIn(10, "Leyton", 3)
	c.CheckOut(10, "Paradise", 8)
	fmt.Println(c.GetAverageTime("Leyton","Paradise"))
}
...
func (c *count) GetAverageTime()  float64{
	return c.sum/c.count
}

func (c *count) UpdateSum(sum int)  {
	if c == nil {
		c = &count{}
	}
	 c.sum += float64(sum)
...
func (this *UndergroundSystem) CheckOut(id int, stationName string, t int)  {
	...
	if _, ok:= this.sc[s];!ok {
		this.sc[s] = &count{}
	}
	this.sc[s].UpdateSum(t-in.t)
}


func (this *UndergroundSystem) GetAverageTime(startStation string, endStation string) float64 {
	return this.sc[station{
		in:startStation,
		out:endStation,
	}].GetAverageTime()
}
