...
func shortestCompletingWord(licensePlate string, words []string) (result string) {
    //过滤数字空格，并统计字母出现的数字，并转为小写
    licensePlateMap := getShowFrequency(licensePlate)
    //比对后面的words是否符合
    for _, word := range words {
        if result != "" && len(word) >= len(result) {
            continue
        }
        isShortestCompletingWord := true
        wordMap := getShowFrequency(word)
        for letter, num := range licensePlateMap {
            if wordMap[letter] < num {
                isShortestCompletingWord = false
            }
        }

        if isShortestCompletingWord {
            result = word
        }
...
//获取各字面的出现次数
func getShowFrequency(licensePlate string) map[string]int {
