...
func minCostSetTime(startAt int, moveCost int, pushCost int, targetSeconds int) int {
	...
	//计算每个的时间，找出最小
	minR := math.MaxInt
	fen := targetSeconds / 60
	miao := targetSeconds % 60
	for _, v := range getAll(fen, miao) {
		minR = min(minR, getOneCostSetTime(startAt, moveCost, pushCost, v))
	}
...
func getAll(fen int, miao int) []string {
	result := make([]string, 0)
	if fen >= 100 {
		return result
	}
	if fen == 0 {
		result = append(result, fmt.Sprintf("%d", miao))
	} else {
		result = append(result, fmt.Sprintf("%d%02d", fen, miao))
	}
	return result
}

func getOneCostSetTime(lastAt int, moveCost int, pushCost int, target string) int {
	r := 0
	lastAtS := fmt.Sprintf("%d", lastAt)
